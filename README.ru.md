...
- индикатор токенов, стоимости, остатка бюджета сообщения и заполнения контекста;
...
Текущий режим, модель, usage и остаток лимитов сообщения отображаются в нижней панели, а
заполняющаяся слева направо колба контекста — справа. Завершённый вывод переносится в обычный
...
Основные параметры: `model`, `effort`, `thinking`, `maxTokens`, `budget`, `diagnostics`, `theme`,
`permissions`, `providers` и `mcp`. Секция `budget` задаёт лимиты запросов к модели, токенов,
...
автоопределение списком до восьми команд. Обычно редактировать конфиг вручную не требуется —
