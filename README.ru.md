...
Текущий режим, модель, usage и заполняющаяся слева направо колба контекста отображаются справа в
нижней панели. Завершённый вывод переносится в обычный scrollback терминала, поэтому историю можно
плавно листать даже во время потокового ответа.
...
Основные параметры: `model`, `effort`, `thinking`, `maxTokens`, `budget`, `theme`, `permissions`,
`providers` и `mcp`. Секция `budget` задаёт лимиты запросов к модели, токенов, примерной стоимости
(если провайдер сообщает цены) и субагентов на одно сообщение. Обычно редактировать конфиг вручную
не требуется — провайдер можно добавить при первом запуске или через `/login`.
