...
| `/attach <путь>` · `/attach clear` | Прикрепить картинку или UTF-8 файл к следующему сообщению. |
...
оставляя последние три пользовательских хода. Команда `/compact` запускает это вручную.
...
Содержимое отправляется провайдеру только вместе со следующим сообщением.
